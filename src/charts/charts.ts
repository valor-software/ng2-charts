  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if (this.initFlag) {
      ...
      if (changes.hasOwnProperty('data') || changes.hasOwnProperty('datasets') || changes.hasOwnProperty('labels')) {
        ...
        if (changes['labels']) {
          this.chart.config.data.labels = changes['labels'].currentValue;
        }
...
function formatGeneratedColor(colors: Array<number>): Color {
...
function getColors(chartType: string, index: number, count: number): Color {
  ...
  return formatGeneratedColor(generateColor(index));
}
